...
//------------------------------
  ...
  bool load(const char* AFilename) {
    MIP_Print("Loading ladspa plugin: %s\n",AFilename);
    if (MLibrary.loadLib(AFilename)) {
      MIP_Print("ladspa plugin loaded\n");
      MGetLadspaDescriptor = (LADSPA_Descriptor_Function)MLibrary.getLibSymbol("ladspa_descriptor");
      MIP_Print("MGetLadspaDescriptor = %p\n",MGetLadspaDescriptor);
      return true;
    }
    MIP_Print("error loading ladspa plugin\n");
    return false;
  }
