#ifndef mip_midi_file_included
#define mip_midi_file_included
//----------------------------------------------------------------------

//#include <vector>
#include "base/types/mip_array.h"
#include "midi/mip_midi.h"
...
//------------------------------
  ...
  MIP_MidiTrack*    MCurrentTrack    = nullptr;
...
//------------------------------
  ...
  MIP_MidiFile() {
    MSequence = new MIP_MidiSequence();
  }
...
//------------------------------
  ...
  MIP_MidiSequence* getMidiSequence() {
    return MSequence;
  }
  ...
  void print_event(uint32_t i, MIP_MidiEvent* event) {
    if (event->msg1 < 0xF0) {
...
*/
  ...
  void start_new_track() {
    MIP_MidiTrack* track = new MIP_MidiTrack();
...
//------------------------------
  ...
  */
  ...
  void read_event(uint32_t ADeltaTime) {
    ...
    uint8_t        value       = 0;
    uint8_t        event       = 0;
    MIP_MidiEvent* midi_event  = nullptr;
    ...
    switch (event & 0xf0) {
      case 0x80:
        index = read_byte();
        value = read_byte();
        midi_event = new MIP_MidiEvent(ADeltaTime,event,index,value);
    ...
    switch (AEvent) {
      ...
      */
      ...
      case 0xF0:
        ...
        midi_event = new MIP_MidiEvent(ADeltaTime,AEvent,0,0);
      ...
      */
      ...
      default:
        ...
        midi_event = new MIP_MidiEvent(ADeltaTime,0,0,0);
        MCurrentTrack->events.push_back(midi_event);
        break;
    } // switch
  }
