...
#define MIP_VOICE_NUM_NOTE_BUFFERS  16
//----------
...
/*
  each voice calc to their own buffer,
  MIP_VoiceBuffers[MIP_VOICE_BUFFERSIZE * index]
  possiblbly post-process each voice separately)
  Add/mul (scale) to final buffer
  and again, possibly post-process (gobal)
*/

__MIP_ALIGNED(MIP_ALIGNMENT_CACHE)
float MIP_VoiceBuffers[MIP_VOICE_BUFFERSIZE * MIP_VOICE_NUM_NOTE_BUFFERS] = {0};
...
//------------------------------
  ...
  void voiceNoteOff(int32_t AChannel, int32_t AKey, float AVelocity) {
    ...
    if (note >= 0) {
      ...
      MIP_Print("no voice playing for chan %i key %i\n",AChannel,AKey);
    }
  }
