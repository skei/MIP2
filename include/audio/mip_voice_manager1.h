...
/*
  - one note_on can start multiple voices?
    shoud MNoteToVoice be MNotesToVoices, and be a 2d array..
...
//------------------------------
  ...
  uint32_t                    MPrevVoiceState[NUM]          = {0};
  ...
  int32_t                     MNoteState[MIP_VOICE_NOTES]   = {0};
...
//------------------------------
  ...
  void queueNoteEnd(uint32_t note) {
    MNoteEndQueue.write(note);
  }

  //----------

  void flushNoteEnds() {
    MIP_Assert(MOutEvents != nullptr);
    uint32_t note = 0;
    while (MNoteEndQueue.read(&note)) {
      uint32_t channel  = note  >> 7;
      uint32_t key      = note & 127;
      MIP_Assert(channel < 16);
      MIP_Assert(key < 128);
      MIP_Print("NOTE_END ch %i key %i\n",channel,key);
      clap_event_note_t event;
      clap_event_header_t* header = (clap_event_header_t*)&event;
      header->size      = sizeof(clap_event_note_t);
      header->time      = MVoiceContext.process->frames_count - 1;// header->time;
      ...
      event.velocity    = 0.0;
      MOutEvents->try_push(MOutEvents,header);
  ...
  void reset() {
    for (uint32_t i=0; i<(MIP_VOICE_NOTES); i++) {
      MNoteToVoice[i] = -1;
      MNoteState[i] = MIP_VOICE_OFF;
    }
    for (uint32_t i=0; i<NUM; i++) {
      MPrevVoiceState[i]  = MIP_VOICE_OFF;
      ...
      case CLAP_EVENT_NOTE_ON:
        {
          ...
          MIP_Print("NOTE_ON port %i ch %i key %i\n",note_event->port_index,note_event->channel,note_event->key);
          bool started_voice = on_note_on(note_event);
          if (!started_voice) {
            MIP_Print("on_note_on = false\n");
            uint32_t note = (note_event->channel * 128) + note_event->key;
            queueNoteEnd(note);
          }
        }
        break;
...
//------------------------------
  ...
  */
  ...
  bool handle_voice_note_on(int32_t APort, int32_t AChannel, int32_t AKey, float AVelocity) {
    //uint32_t note = (AChannel * 128) + AKey;
    //MNoteState[note] = MIP_VOICE_PLAYING;
    int32_t voice = start_voice(APort,AChannel,AKey,AVelocity);
    if (voice < 0) return false;
    MIP_Print("Starting voice %i\n",voice);
    return true;
  }
...
//------------------------------
  ...
  void reset_voice(int32_t voice) {
    MIP_Print("Resetting voice %i\n",voice);
    if (voice < 0) return;
