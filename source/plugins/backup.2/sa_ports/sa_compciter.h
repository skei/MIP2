...
//------------------------------
  ...
  void recalc(float srate) {
    need_recalc = false;
    threshDB  = - MParameterValues[0];
    thresh    = exp(threshDB / c);
    ratio     = 1 / 20;
    release   = exp(-60 / (pow(1 - MParameterValues[1] / 100, 3) * 500 * srate / 1000) / c);
    t0        = 0;
    t1        = 0;
    blp       = -exp(-2 * MIP_PI * MParameterValues[2] * 3 / srate);
    alp       = 1.0 + blp;
    wet       = exp(MParameterValues[3] / c) / exp((threshDB - threshDB * ratio ) / c);
    dry       = exp(MParameterValues[4] / c);
    seekgain  = 1;
  }
...
//------------------------------
  ...
  bool init() final {
    appendParameter(new MIP_Parameter( 0, CLAP_PARAM_IS_AUTOMATABLE, "Drive", "",  0,   60,    0    ));
    appendParameter(new MIP_Parameter( 1, CLAP_PARAM_IS_AUTOMATABLE, "Dist",  "",  0,   100,   25   ));
    appendParameter(new MIP_Parameter( 2, CLAP_PARAM_IS_AUTOMATABLE, "HP",    "",  800, 12000, 5000 ));
    appendParameter(new MIP_Parameter( 3, CLAP_PARAM_IS_AUTOMATABLE, "Wet",   "", -60,  24,   -6    ));
    appendParameter(new MIP_Parameter( 4, CLAP_PARAM_IS_AUTOMATABLE, "Dry",   "", -120, 0,     0    ));
