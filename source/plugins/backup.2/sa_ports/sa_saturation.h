...
//------------------------------
  ...
  void recalc(void) {
    ...
    a = MIP_Max(0, MIP_Min(1, MParameterValues[0]));
    if (a > 0) {
      gain = 1 - (1 - 1/(float)MParameterValues[1]) * a;
      float f = 1.0f;
      switch( (int)MParameterValues[2] ) {
        case 0:
          a *= MIP_PI05;
          f = sin(a);
          break;
  ...
  void handle_process(const clap_process_t *process) final {
    ...
    for (uint32_t i=0; i<len; i++) {
      ...
      if (a>0) {
        ...
        switch( (int)MParameterValues[2] ) {
          case 0:
            for (uint32_t j=0; j<MParameterValues[1]; j++) {
              x0 = sin(x0 * a) * div;
              x1 = sin(x1 * a) * div;
            }
            break;
...
//------------------------------
  ...
  bool init() final {
    ...
    appendParameter(new MIP_Parameter(     0, CLAP_PARAM_IS_AUTOMATABLE, "Saturation", "",  0,   1,    0    ));
    appendParameter(new MIP_IntParameter(  1, CLAP_PARAM_IS_AUTOMATABLE, "Stages",     "",  1,   10,   1    ));
    appendParameter(new MIP_TextParameter( 2, CLAP_PARAM_IS_AUTOMATABLE, "Function",   "",  0,   1,    0, txt_func ));
