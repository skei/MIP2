...
const char* sonic_maximizer_features[] = {
  "audio_effect",
  nullptr
};
...
//------------------------------
  ...
  sonic_maximizer_plugin(const clap_plugin_descriptor_t* ADescriptor, const clap_host_t* AHost)
  : MIP_Plugin(ADescriptor,AHost) {
  }

  //----------

  virtual ~sonic_maximizer_plugin() {
  }
...
MIP_ClapPlugin* MIP_CreatePlugin(uint32_t AIndex, const clap_plugin_descriptor_t* ADescriptor, const clap_host_t* AHost) {
  switch (AIndex) {
    case 0: return new sonic_maximizer_plugin(ADescriptor,AHost);
  }
  return nullptr;
}
